...
export const unmarksComplete = async (req, res) => {
    try {
        const { habitId, date } = req.body;
        const completedDate = date || todayKey();
...
export const getHeatmap = async (req, res) => {
    try {
        ...
        for (const l of logs) count[l.completedDate] = (count[l.completedDate] || 0) + 1;
        const data = days.map((d) => ({ date:d, count: count[d] || 0 }));
        res.json(data);
    } catch (err) {
        res.status(500).json({ message: err.message});
    }
